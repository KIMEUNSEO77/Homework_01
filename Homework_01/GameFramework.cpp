...
// 화면 깜빡임을 줄이기 위해 윈도우 클라이언트 영역 디바이스 컨텍스트를 사용해서
// 직접 클라이언트 표면에 그리지 않음

// 클라이언트 표면을 대신할 비트맵 생성, 이 비트맵을 메모리 디바이스 컨텍스트로 생성
...
// 렌더링 시작 전에 비트맵 표면을 원하는 색상으로 지움
// 씬을 화면, 클라이언트 영역으로 렌더링하기 위해 먼저 씬의 게임 객체들을 비트맵 표면으로 렌더링
...
// 비트맵 표면으로 렌더링된 비트맵을 클라이언트 영역으로 복사해서 화면에 보여줌
